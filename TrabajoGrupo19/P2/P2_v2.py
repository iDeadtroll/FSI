...
class Jugador:
    ...
    def __str__(self):
        campos = ["ID","JUGADOR","NACIONALIDAD","POSICION","CLUB","EDAD","NACIMIENTO","PARTIDOS_JUGADOS","PARTIDOS_TITULAR","MINUTOS_JUGADOS","PARTIDOS_COMPLETOS","GOLES","ASISTENCIAS","G+A","G+PK","PK"]
    ...
    def validar_datos_jugador(self, id: int):
        ...
        while not valido:
            ...
            if not (nacionalidad.isalpha() and len(nacionalidad) == 3):
                print(
                    "Error: nacionalidad debe tener 3 caractéres (A-Z).Ejemplo: ESP, ITA, FRA..."
                )
                count += 1
                continue
            ...
            if not (born.isdigit() and 1980 <= int(born) <= 2024):
                print(
                    "Error: el año de nacimiento debe ser un número entre 1980 y 2024."
                )
                count += 1
                continue
