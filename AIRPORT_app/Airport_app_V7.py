...
class Avion(threading.Thread):
    ...
    def run(self):
        ...
        print(f"Avión [{self.id}] {self.tipo.upper()} solicita una pista para {self.operacion}.")
        mutex.acquire()
...
def main(m, n):
    aviones = [Avion(i, "normal") for i in range(m)] + [Avion(i + m, "vip") for i in range(n)]
...
if __name__ == "__main__":
    main(10, 6)  # 6 aviones normales, 4 aviones VIP